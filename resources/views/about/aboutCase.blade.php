            ...
            <div class="row s_product_inner">
                ...
                <div class="col-lg-5 offset-lg-1">
                    <div class="s_product_text">
                        <h3>{{$case->name}}</h3>
                        <h2 style="color: #2a9055;">{{$case->price}}₴</h2>
                        <ul class="list">
                            @if($case->count>0)
                            ...
                            @endif
                            @auth('web')
                                <li><a href="{{route('addFavorite',$case->product_id)}}">Додати до вподобань</a></li>
                            @endauth
                        </ul>
                        <div class="product_count">
                            <form method="post" action="{{route('addToBasketSubmit',[$case->product_id])}}" class="p-5">
                                @csrf
                                @if($case->count>0)
                                    <span>Кількість:</span>
                                    <input type="number" name="count" value="1" style="width: 11%" min="1" max="{{$case->count}}">
    ...
    <div class="container p-4">
        ...
        <div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade" id="history" role="tabpanel" aria-labelledby="history-tab">
                @include('feedback_layout',array('some'=>$case->product_id))
            </div>
    </div>
@endsection
